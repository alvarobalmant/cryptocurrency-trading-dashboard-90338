-- Corrigir função get_employee_commission_summary
-- Bug fix: pagamentos de períodos não devem ser descontados do saldo devendo
...
BEGIN
  ...
  WITH employee_stats AS (
    SELECT 
      ...
      -- ✅ Apenas agendamentos que NÃO estão em períodos
      COALESCE(SUM(
        CASE 
  ...
  paid_commissions AS (
    ...
    WHERE cp.barbershop_id = barbershop_id_param 
      AND cp.status = 'paid'
      AND cp.commission_period_id IS NULL  -- ✅ CORREÇÃO: Apenas pagamentos diretos, não de períodos
...
'Sistema Unificado de Comissões: calcula apenas agendamentos que NÃO estão em commission_period_services. Pagamentos de períodos (commission_period_id NOT NULL) não são descontados porque os agendamentos já saíram da conta ao entrarem no período. Apenas pagamentos diretos (commission_period_id IS NULL) são descontados do saldo devendo.';