-- Fix calculate_product_total_volume for simple products (no variants)
CREATE OR REPLACE FUNCTION public.calculate_product_total_volume(
  p_product_id uuid,
  p_barbershop_id uuid DEFAULT NULL::uuid
)
RETURNS TABLE(total_units integer, total_volume numeric, total_value numeric, variants jsonb)
LANGUAGE plpgsql
