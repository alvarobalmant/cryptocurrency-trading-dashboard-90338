-- Atualizar função get_employee_commission_summary para considerar deduções de adiantamentos
...
BEGIN
  ...
  WITH employee_stats AS (
    SELECT 
      ...
      -- Total de comissões de agendamentos confirmados e pagos
      COALESCE(SUM(
        CASE 
      ...
      -- Comissões de agendamentos pendentes (futuras)
      COALESCE(SUM(
        CASE 
  ...
  -- NOVO: Buscar deduções (adiantamentos pagos)
  deductions_total AS (
    SELECT 
      cd.employee_id,
    ...
    WHERE cd.barbershop_id = barbershop_id_param
      AND cd.applied_to_period_id IS NULL  -- Deduções ainda não aplicadas a um período fechado
  ...
  SELECT 
    ...
    es.emp_name,
    es.emp_commission,
    -- CORRIGIDO: Total ganho MENOS pagamentos MENOS deduções (adiantamentos)
    GREATEST(
    ...
    es.future_pending,
    COALESCE(pc.total_paid, 0) + COALESCE(dt.total_deductions, 0) as total_paid_with_advances,
