-- Atualizar função compute_daily_availability para mostrar todos os horários
-- Slots disponíveis = true, bloqueados/ocupados = false
...
BEGIN
  ...
  -- Mark breaks as false (not available)
  FOR v_break IN
  ...
  LOOP
    ...
    WHILE v_current_time < v_break.end_time LOOP
      -- Mark as false instead of removing
      v_slots := v_slots || jsonb_build_object(to_char(v_current_time, 'HH24:MI'), false);
      v_current_time := v_current_time + interval '10 minutes';
    END LOOP;
  END LOOP;
  
  -- Mark appointments as false (not available)
